...
if __name__ == "__main__":
    ...
    img_src = cv2.imread('/Users/han/generate_data/sku/roi/6-11.jpg')
    img_mask = cv2.cvtColor(img_src , cv2.COLOR_BGR2GRAY)
    img_src = img_src.astype(np.float64)

    img_target = cv2.imread('/Users/han/generate_data/sku/background/0.png')
    ...
    img_target_2 = cv2.imread('/Users/han/generate_data/sku/background/0.png')
    img_src2 = cv2.imread('/Users/han/generate_data/sku/roi/6-11.jpg')


    mask_h , mask_w = img_mask.shape[:2]
    roi = img_target_2[offset[0]:offset[0] + mask_h , offset[1]:offset[1] + mask_w]
    for i in range(mask_h):
        for j in range(mask_w):
            if int(np.sum(img_src2[i][j])) <= 20:
                continue
            else:
                roi[i][j] = img_src2[i][j]


    cv2.imshow("img_target" , img_target_2)
    cv2.waitKey(0)
