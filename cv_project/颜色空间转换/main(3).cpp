...
#define     ONETWO(L, r, c, col) (L[(r) * (col) + c])//将矩阵展开成单列
int find(int set[], int x)//将x设为set[x]
{
	int r = x;
	while (set[r] != r)
		r = set[r];
...
int bwlabel(IplImage* img, int n, int* labels)
{
	if (n != 4 && n != 8)
		n = 4;
	int nr = img->height;//nr为影像高
	int nc = img->width;//nc为影像宽
	int total = nr * nc;
	// results
	memset(labels, 0, total * sizeof(int));//将已开辟内存空间labels的前total个值设为0
	...
	for (int r = 0; r < nr; r++)
	{
		for (int c = 0; c < nc; c++)
		{
			if (ELEM(img, r, c))   // if A is an object,r为高，c为列
			{
				...
				if (c == 0)
					...
					B = find(lset, ONETWO(labels, r, c - 1, nc));//如果lset中像素值不等于该位置的标记labels[r*nc+(c-1)]，则B的值为该处像素值
				if (r == 0)
					C = 0;
				else
...
int main()
{
	IplImage* pImg = cvLoadImage("2.bmp");



	int num = 0;
	int rows = pImg->width;
	int ncols = pImg->height;
	int numodlabel = rows*ncols;
	//int* labels = new int[numodlabel];   // label table
	int* labels = new int[numodlabel];   // label table
	num = bwlabel(pImg, 8, labels);


	std::vector<int> copynum;
	for (int i = 0; i < numodlabel; i++)
	{
		copynum.push_back(labels[i]);
	}
	int max_num = 0;
	for (int i = 0; i < numodlabel; i++)
	{
		if (copynum.at(i) > max_num)
		{
			max_num = copynum.at(i);
		}
	}
	uchar *dstdata = (uchar*)pImg->imageData;
	int datagray = 0;

	std::cout << "八邻域连通区域共有：" << num << std::endl;
	std::cout << "八邻域连通区域共有：" << max_num << std::endl;
	system("pause");
	//delete[] labels;
